...
public class DiseaseDetailsListAdapter extends RecyclerView.Adapter<DiseaseDetailsListAdapter.LoanTypeViewHolder> implements Filterable {
    private Context context;
    List<DiseaseTable> rawDataTableList;
    List<DiseaseTable> rawDataTableListFiltered;
//    List<AddD20Table> rawDataTableList;
//    List<AddD20Table> rawDataTableListFiltered;
List<D20DiseaseTable> rawDataTableListFilteredD20;
    SyncCallbackInterface syncCallbackInterface;
    AppHelper appHelper;
    AppViewModel viewModel;

    public DiseaseDetailsListAdapter(Context context, List<DiseaseTable> rawDataTableList,List<D20DiseaseTable> d20WeedTableList,
                                     SyncCallbackInterface syncCallbackInterface,
...
//            loanTypeViewHolder.txtName.setText(rawDataTableListFilteredD20.get(i).getName());
...
//            });
    ...
    private boolean isAvailable(DiseaseTable seq, TextView checkBox){
        for(int i=0;i<rawDataTableListFilteredD20.size();i++){
            if(rawDataTableListFilteredD20.get(i).getDiseasesId().equals(String.valueOf(seq.getId()))){
...
//                if(rawDataTableListFilteredD20.get(i).getActive().equals("true")) {
    ...
    private boolean isAvailableIdentified(DiseaseTable seq, TextView checkBox){
        for(int i=0;i<rawDataTableListFilteredD20.size();i++){
            if(rawDataTableListFilteredD20.get(i).getDiseasesId().equals(String.valueOf(seq.getId()))){
                if(rawDataTableListFilteredD20.get(i).getActive()) {
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (charString.isEmpty()) {
                    ...
                    List<DiseaseTable> filteredList = new ArrayList<>();
                    ...
                    List<DiseaseTable> filteredList = new ArrayList<>();
                    for (DiseaseTable row : rawDataTableList) {
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                rawDataTableListFiltered = (List<DiseaseTable>) filterResults.values;
...
//    }
    ...
    public interface SyncCallbackInterface {
        void openScreenCallback(int position, DiseaseTable farmerTable, List<DiseaseTable> farmer, String applicationType);

        void updateItemCallback(int position, DiseaseTable applicationStatusTable, String strFarmerID);

        void addPlotDetailsCallback(int position, DiseaseTable applicationStatusTable, String strFarmercode);
    }
}
