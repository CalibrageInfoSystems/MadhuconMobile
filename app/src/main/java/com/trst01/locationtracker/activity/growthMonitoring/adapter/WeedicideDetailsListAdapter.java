...
public class WeedicideDetailsListAdapter extends RecyclerView.Adapter<WeedicideDetailsListAdapter.LoanTypeViewHolder> {
    private Context context;
    List<WeedTable> rawDataTableList;
    List<WeedTable> rawDataTableListFiltered;
    List<D20WeedTable> rawDataTableListFilteredD20;
...
//    List<AddD20Table> rawDataTableListFiltered;
    ...
    public WeedicideDetailsListAdapter(Context context, List<WeedTable> rawDataTableList,List<D20WeedTable> d20WeedTableList,
                                       SyncCallbackInterface syncCallbackInterface,
...
//            if(isAvailable(rawDataTableList.get(i), loanTypeViewHolder.txtCheckBox)){
//                loanTypeViewHolder.txtCheckBox.setEnabled();
//            } else {
//
//            }
            getWeedByID(rawDataTableListFilteredD20.get(i).getWeedId(), loanTypeViewHolder.txtCheckBox);
...
//            isAvailable(rawDataTableList.get(i), loanTypeViewHolder.txtCheckBox);
    ...
    public void getWeedByID(String strVillageId, CheckBox tvFarmerVillage) {
        try {
            viewModel.getWeedById(strVillageId);
            if (viewModel.getInsertWeedIntoLocalDBQueryLiveDataLocalDB() != null) {
                Observer getLeadRawDataObserver = new Observer() {
                    @Override
                    public void onChanged(@Nullable Object o) {
                        WeedTable villageTable = (WeedTable) o;
                        viewModel.getInsertWeedIntoLocalDBQueryLiveDataLocalDB().removeObserver(this);
                ...
                viewModel.getInsertWeedIntoLocalDBQueryLiveDataLocalDB().observe((LifecycleOwner) context, getLeadRawDataObserver);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    private boolean isAvailable(WeedTable seq,CheckBox checkBox){
        for(int i=0;i<rawDataTableListFilteredD20.size();i++){
            if(rawDataTableListFilteredD20.get(i).getWeedId().equals(String.valueOf(seq.getId()))){
                if(rawDataTableListFilteredD20.get(i).getIsActive().equals("true")) {
...
//                    List<WeedTable> filteredList = new ArrayList<>();
...
//                } else {
//                    List<WeedTable> filteredList = new ArrayList<>();
//                    for (WeedTable row : rawDataTableList) {
...
//                rawDataTableListFiltered = (List<WeedTable>) filterResults.values;
...
//    }
    ...
    public interface SyncCallbackInterface {
        void openScreenCallback(int position, WeedTable farmerTable, List<WeedTable> farmer, String applicationType);

        void updateItemCallback(int position, D20WeedTable applicationStatusTable,List<D20WeedTable> d20WeedTableList, String strFarmerID);

        void addPlotDetailsCallback(int position, WeedTable applicationStatusTable, String strFarmercode);
    }
}
